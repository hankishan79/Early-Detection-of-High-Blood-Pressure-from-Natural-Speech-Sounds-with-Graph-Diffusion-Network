...
for k in range(1):
    ...
    for l in range(1):
        ...
        data_1 = pd.read_csv(r'C:\Users\hankishan\Desktop\S_H_B_Database_10132022\pyhton codes\new_train.csv')
        data_1.head()
        ...
        y = data_1['DBP-2CLASSES']
        X = data_1.drop(columns=['MIXED-CLASSES'], axis=1)
        # ****************SMOTE*****************************************************************************************
        from imblearn.over_sampling import SMOTE
        sm = SMOTE()
        X, y = sm.fit_resample(X, y)
        y = X['DBP']
        X = X.drop(['DBP-2CLASSES', 'SBP-2CLASSES','BPM', 'SBP', 'DBP','GENDER','AGE','WEIGHT','HEIGHT'], axis=1)
        # Create a GroupShuffleSplit object
        group_split = GroupShuffleSplit(n_splits=1, test_size=0.2, random_state=42)
        # Define the features and target variables
        features1 = X  # Assuming 'PAT_ID' is the column containing group information
        target = y
        ...
        y2 = data_2['DBP-2CLASSES']
        X2 = data_2.drop(columns=['MIXED-CLASSES'], axis=1)
        # ********SMOTE ************************************************************************************************
        sm2 = SMOTE()
        X2, y2 = sm2.fit_resample(X2, y2)
        y2 = X2['DBP']
        X2 = X2.drop(['DBP-2CLASSES', 'SBP-2CLASSES','BPM', 'SBP', 'DBP','GENDER','AGE','WEIGHT','HEIGHT'], axis=1)
        # Create a GroupShuffleSplit object
        group_split2 = GroupShuffleSplit(n_splits=1, test_size=0.2, random_state=42)

        # Define the features and target variables
        features2 = X2  # Assuming 'PAT_ID' is the column containing group information
        target2 = y2

        # Generate the train/validation/test indices
        train_idx2, val_test_idx2 = next(group_split2.split(features2, target2, groups=X2['PAT_ID']))
        val_idx2, test_idx2 = next(group_split2.split(features2.iloc[val_test_idx2], target2.iloc[val_test_idx2],
                                                   groups=X2.iloc[val_test_idx2]['PAT_ID']))
        # Split the data into train, validation, and test sets
        X_train2, y_train2 = features2.iloc[train_idx2], target2.iloc[train_idx2]
        X_val2, y_val2 = features2.iloc[val_idx2], target2.iloc[val_idx2]
        X_test2, y_test2 = features2.iloc[test_idx2], target2.iloc[test_idx2]

        X_train2 = X_train2.drop(['PAT_ID'], axis=1)
        X_test2 = X_test2.drop(['PAT_ID'], axis=1)
        X_val2 = X_val2.drop(['PAT_ID'], axis=1)

        # ***************************************************************************************************************
        # ***********************CONCAT DATA*****************************************************************************
        frm1=[X_train,X_train2]
        X_train=pd.concat(frm1)
        frm2=[y_train,y_train2]
        y_train=pd.concat(frm2)

        frm3=[X_val,X_val2]
        X_val=pd.concat(frm3)
        frm4=[y_val,y_val2]
        y_val=pd.concat(frm4)

        frm5=[X_test,X_test2]
        X_test=pd.concat(frm5)
        frm6=[y_test,y_test2]
        y_test=pd.concat(frm6)
        # ***************************************************************************************************************
        # ********************************* feature Selection with featureWiz ******************************************
        from featurewiz import FeatureWiz
        features = FeatureWiz(corr_limit=0.95, feature_engg='', category_encoders='',dask_xgboost_flag=False, nrows=None, verbose=0)
        ...
        X_val = object.transform(X_val)
        # ***************************************************************************************************************
        # *************  mmHg to cmHg conversion (x/10 division )********************************************************
        ...
        print("testtest Min : ", y_val.min(), "Max : ", y_val.max())
        print("count : ", k, l)
        ...
        regressor.fit(encoded_train, y_train, epochs=100, batch_size=16,verbose=0)
